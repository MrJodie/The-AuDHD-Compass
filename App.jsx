...
// The purpose of this hook is to manage Firebase initialization and authentication state once.
const useFirebase = () => {
    ...
    useEffect(() => {
        if (!appInitialized && Object.keys(firebaseConfig).length > 0) {
            try {
                ...
                setLogLevel('debug'); // Enable debug logging for visibility
                ...
                const attemptSignIn = async () => {
                    ...
                    if (initialAuthToken) {
                        try {
                            // Attempt custom token sign-in (Canvas default)
                    ...
                    if (!user) {
                        try {
                            // Anonymous sign-in (necessary fallback for public read/write rules)
                    ...
                    onAuthStateChanged(authInstance, (user) => {
                        if (user) {
                            ...
                            // Should not happen after successful signInWithCustomToken or signInAnonymously
                            setUserId(null); 
                        }
                        setIsAuthReady(true);
                    });
                ...
                setIsAuthReady(true); // Treat as ready to stop loading state, even on failure
            }
        }
    }, [appInitialized]); // Run once on component mount
...
// Categories based on common AuDHD challenges and interests
...
// 1. Thread Renderer (Recursive for Reddit-style discussions)
const CommentRenderer = React.memo(({ comment, allComments, db, userId, threadId }) => {
    ...
    const handlePostReply = async () => {
        ...
        try {
            // New comments are added to the 'comments' subcollection of the parent thread
            await addDoc(collection(db, `artifacts/${appId}/public/data/community_threads/${threadId}/comments`), {
                content: replyText.trim(),
                userId: userId,
                timestamp: serverTimestamp(),
                parentId: comment.id, // Reference to the comment being replied to
                // We don't track the category here, as it belongs to the parent thread
            });
            setReplyText('');
...
const Discussions = ({ db, userId }) => {
    ...
    const [selectedThread, setSelectedThread] = useState(null); // The thread currently being viewed

    // 1. Fetch Threads based on Category
    useEffect(() => {
        if (!db) return;
        ...
        // Listen for real-time updates on threads
    ...
    // 2. Fetch Comments when a thread is selected
    useEffect(() => {
        if (!db || !selectedThread) return;

        // Function to listen to comments for a specific thread
    ...
    const handleCommentPost = async (threadId, content) => {
        ...
        try {
            ...
            // Note: Firebase rules will manage permissions, not the client-side code
        } catch (error) {
            console.error("Error posting comment:", error);
        }
    };

    // Render the Thread Viewer or the Thread List
    if (selectedThread) {
        // Find top-level comments (those without a parentId)
...
const Chat = ({ db, userId }) => {
    ...
    useEffect(() => {
        ...
        // Query ordered by timestamp, limiting to last 50 messages for performance
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            // Sort client-side if query ordering is complex or not fully indexed
    ...
    return (
        <div className="flex flex-col h-[70vh] bg-gray-50 border-t border-gray-200">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
                ...
                {messages.slice(-100).map(msg => ( // Display last 100 messages
...
const Wiki = ({ db, userId }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            setPages(fetchedPages.sort((a, b) => a.title.localeCompare(b.title))); // Sort alphabetically
        }, (error) => {
        ...
        <div className="min-h-screen bg-gray-100 font-sans">
            ...
            <header className="bg-white shadow-md">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center">
                    <div className="flex items-center space-x-4">
                        ...
                        <p className="text-sm text-gray-500 hidden sm:block">Built by us, for us.</p>
                    </div>
                    <div className="mt-3 sm:mt-0 flex flex-col items-end">
                        <p className="text-xs text-gray-600">Your Anonymous ID:</p>
                        <p className="font-mono text-sm font-semibold text-gray-900 bg-gray-100 px-2 py-1 rounded-md">
                    ...
                    <div className="flex space-x-4">
                        {tabs.map(tab => (
                            <button
                                ...
                                className={`py-3 px-4 text-sm font-medium transition duration-200 
                                    ${activeTab === tab 
            ...
            <footer className="bg-gray-800 text-white py-4 mt-8">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-sm">
                    &copy; 2025 The AuDHD Compass Community. All rights reserved.
                </div>
            </footer>
        </div>
    );
